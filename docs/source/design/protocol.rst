...
- If the peripheral is executing a sequence of actions due to a command from some channel but receives a subsequent command on that same channel, it will stop executing the previous sequence of actions from that previous command and start executing the new sequence of actions from the new command. For example, if the peripheral is in the middle of moving an actuator to some position but receives a new command to move that actuator to a new position, the peripheral will only take actions to move the actuator to the new position.
