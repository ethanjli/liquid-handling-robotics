...
class Batch(Batch):
    ...
    async def test_routine(self):
        ...
        await self.protocol.blink.request_high_interval()
        await self.protocol.blink.request_low_interval()
        await self.protocol.blink.request_periods()
        await self.protocol.blink.request_notify()

        print('RPC with complete wait for finite-periods blink:')
        await self.protocol.blink.request_notify(1)
        await self.protocol.blink.request_low_interval(500)
        await self.protocol.blink.request_high_interval(500)
        ...
        await self.protocol.blink.request_notify(0)
        await self.protocol.blink.request_low_interval(100)
        await self.protocol.blink.request_high_interval(100)
